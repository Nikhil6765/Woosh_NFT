...
export function Card({ data, username }) {
  ...
  const [pngImage, setPngImage] = React.useState(defaultImage);
  const { name, avatar_url, bio, followers, public_repos } = data;
  ...
  }, [avatar_url, bio, followers, name, public_repos, username]);
  ...
  const mintNFT = async () => {
    try {
      ...
      const contractAddress = "0xf373dfD3d34609286a0F931c009c0b07fC5e8302";
      const contractABI = [
        {
          "inputs": [
            {
